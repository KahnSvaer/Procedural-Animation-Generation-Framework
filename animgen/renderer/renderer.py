...
def colormap_faces(faces, background=np.array([255, 255, 255])) -> Image.Image:
    ...
    palette = RandomState(0).randint(40, 255, (np.max(faces + 2), 3)) # must init every time to get same colors
    #print(palette)
...
class Renderer:
    ...
    def __init__(self, viewport_width = 1024, viewport_height = 1024):
        """
        """
        self.renderer = pyrender.OffscreenRenderer(viewport_height=viewport_height, viewport_width=viewport_width)
        self.shaders = {
    ...
    def set_object(self, source: trimesh.Trimesh | GeneratedAssetClass | str | Path, smooth=False):
        """
        """
        ...
        assert isinstance(self.tmesh, trimesh.Trimesh), f'Invalid mesh type {type(self.tmesh)}'
...
def render_multiview(
    renderer: Renderer | None = Renderer(),
    ...
    assert camera_generation_method in POSITION_GENERATORS, \
    f"Invalid camera generation method {camera_generation_method}. Must be one of {list(POSITION_GENERATORS.keys())}"
